...
extern int FUNC206LXDEV109(int interruptId);
...
typedef int (*GLJOSDevCreate)(void *pDev, const char *filename, int mode);
typedef int (*GLJOSDevOpen)(void *pDev, const char *filename, int flag, int mode);
typedef int (*GLJOSDevClose)(void *pDev);
typedef int (*GLJOSDevWrite)(void *pDev, const char *pBuf, int nBytes);
typedef int (*GLJOSDevRead)(void *pDev, char *pBuf, int nBytes);
typedef int (*GLJOSDevRemove)(void *pDev, const char *filename);
typedef int (*GLJOSDevIoctl)(void *pDev, int function, unsigned long arg);


GLJ_DEVICEDRV gljosIODevRegister(
		GLJOSDevCreate  pCreate,
		GLJOSDevRemove  pRemove,
		GLJOSDevOpen        pOpen,
		GLJOSDevClose   pClose,
		GLJOSDevRead        pRead,
		GLJOSDevWrite   pWrite,
		GLJOSDevIoctl   pIoctl);


int gljosIODevAdd(GLJ_DEVICEDRV devicedrv, const char *name, void *param);
