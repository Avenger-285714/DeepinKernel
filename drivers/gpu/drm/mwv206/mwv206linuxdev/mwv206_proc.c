...
#include "../mwv206hal/jmpwm.h"
#include <linux/timer.h>
#include <linux/version.h>

#define MAC206LXDEV011 0x400004
#define MAC206LXDEV024 0x1FF0
#define OTP_PRODUCT2 0x1FE0
...
unsigned int FUNC206LXDEV007(V206DEV025 *pDev)
{
	unsigned int ret;
	ret = V206DEV001(MAC206LXDEV024);
	if (FUNC206LXDEV016()) {
		ret = ret & 0xff;
	} else {
		ret = (ret >> 24) & 0xff;
...
static int FUNC206LXDEV014(struct seq_file *seq, void *offset)
{
	...
	subsystemid = V206DEV001(OTP_PRODUCT2);
	if (subsystemid != 0) {
		subsystemid = subsystemid & 0xff;
	} else {
		subsystemid = FUNC206LXDEV007(pDev);
	}
...
/*
static int gpuinfo_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, gpuinfo_show, pde_data(inode));
}
*/

static const struct proc_ops gpuinfo_proc_ops = {
...
int FUNC206LXDEV158(struct pci_dev *V206DEV103)
{
	...
	ent = proc_create_data(gpuName, S_IRUGO, NULL, &gpuinfo_proc_ops, V206DEV103);
	if (!ent) {
		return -ENOMEM;
	}
	gst_timer_pdev = pDev;
