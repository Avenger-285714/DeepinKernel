...
static void FUNC206HAL124(MWV206ChipConfig *pchip)
{
	pchip->corefreq   = 1000;
	...
	pchip->subenable  = (V206API118 | V206API117 | V206API123);
	pchip->pcictrl    = 1;
	pchip->pcilane    = 8;
	pchip->pcibar     = 0;
	pchip->pcirom     = 0;
...
static void FUNC206HAL130(MWV206PortConfig *pport)
{
	uint32_t i = 0;
...
static void FUNC206HAL127(MWV206FbConfig *pcfg)
{
	pcfg->mode         = V206GLJ020;
	pcfg->V206DEV079  = 0;
	pcfg->rfsrate      = 0;
	pcfg->resv         = 0;
	pcfg->htotal       = 0;
...
static void FUNC206HAL126(MWV206DevConfig *pcfg)
{
	int port;
...
static void FUNC206HAL128(MWV206DevConfig *pcfg)
{
	int port;

	FUNC206HAL124(&pcfg->chip);
	FUNC206HAL127(&pcfg->fb);
	pcfg->fb.mode = V206GLJ016;
...
static void FUNC206HAL129(MWV206DevConfig *pcfg)
{
	int port;

	FUNC206HAL124(&pcfg->chip);
	FUNC206HAL127(&pcfg->fb);
	pcfg->fb.mode = V206GLJ020;
	...
	V206DEVCONFIG031(pcfg->lvds[0].flags, 1);
	V206DEVCONFIG041(pcfg->lvds[0].flags, V206DEVCONFIG013);
	pcfg->lvds[0].stdedid = V206GLJ020;
	V206DEVCONFIG032(pcfg->lvds[0].flags, 1);
	V206DEVCONFIG033(pcfg->lvds[0].flags, 0);
	V206DEVCONFIG035(pcfg->lvds[0].flags, 1);

	pcfg->lvds[0].bitmode = V206GLJ046;
...
static void FUNC206HAL125(MWV206DevConfigBin *pbin, MWV206DevConfig *pcfg)
{
	pbin->magic[0] = 'j';
	pbin->magic[1] = 'c';
	pbin->magic[2] = 'f';
...
{
	static MWV206DevConfig cfg;
	static MWV206DevConfigBin bin;
