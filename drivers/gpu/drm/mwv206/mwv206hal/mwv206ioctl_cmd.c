...
{
	if (FUNC206LXDEV105(pDev->V206DEV064[V206API003], timeout) == 0) {
...
int FUNC206HAL300(void *dev, long arg)
{
	...
	if (!FUNC206HAL368(pDev, V206API003)) {
...
int FUNC206HAL404(V206DEV025 *pDev)
{
	...
	if (FUNC206LXDEV105(pDev->V206DEV064[V206API004], MWV206CMDTIMEOUT) == 0) {
...
int FUNC206HAL285(void *dev, const unsigned int *pCmd, int count)
{
	...
	if (FUNC206HAL368(pDev, V206API004)
...
int FUNC206HAL410(V206DEV025 *pDev, long arg)
{
	...
	if (sync->op == SYNC_WAITFORIDLE) {
		...
		return pDev->V206DEV096.wait(pDev, sync->timeout);
