...
static int FUNC206HAL106(MWV206DevConfigBin *pbinaddr)
{
...
MWV206BoardConfig FUNC206HAL066 = {0};
...
{
	MWV206DevConfigBin bhdr;
	MWV206DevConfig    *pcfg;
	MWV206BoardConfig  *pboardcfg;
	...
	if (bdefaultRearder == 0) {
		...
		memcpy(pboardcfg, &FUNC206HAL066, sizeof(MWV206BoardConfig));
		kfree(pbuf);
	}
#endif
...
int FUNC206HAL147(V206DEV025 *pDev, int addr)
{

	uint8_t  inAddr[2] = {'J', 'M'};
	uint8_t  outAddr[2] = {0, 0};
	uint32_t V206IOCTLCMD009 = 0;

	V206IOCTLCMD009 = addr * 1024 * 1024;

	FUNC206HAL229(pDev, 0, outAddr, 2);
	FUNC206HAL229(pDev, V206IOCTLCMD009, inAddr, 2);

	FUNC206HAL228(pDev, outAddr, 0, 2);
...
int FUNC206HAL145(void *dev)
{

	V206DEV025 *pDev = (V206DEV025 *) dev;


	pDev->V206DEV105.chip.V206DEV072 = 128;

	if (!FUNC206HAL147(pDev, 512)) {
		pDev->V206DEV105.chip.V206DEV038 = 1024;
		pDev->V206DEV105.chip.V206DEV039 = 1024;
		pDev->V206DEV105.chip.V206DEV072 = 512;
	}

	if (!FUNC206HAL147(pDev, 1024)) {
		pDev->V206DEV105.chip.V206DEV038 = 2048;
		pDev->V206DEV105.chip.V206DEV039 = 2048;
		pDev->V206DEV105.chip.V206DEV072 = 512;
	}
...
int FUNC206HAL146(void *dev)
{

	uint32_t ddr_level;
	uint32_t chip_qulitylevel;
	uint32_t val[2];
	uint8_t  is7201chip;
	...
	val[0] = V206DEV001(V206DEVCONPARSER003);
	val[1] = V206DEV001(V206DEVCONPARSER003 + 0x4);

	ddr_level = (val[0] >> 24) & 0xff;
	chip_qulitylevel = val[1] & 0xff;

	if (0 != V206DEV001(V206DEVCONPARSER004)) {
		ddr_level = (V206DEV001(V206DEVCONPARSER004) >> 24) & 0xff;
		chip_qulitylevel = V206DEV001(V206DEVCONPARSER004 + 0x4) & 0xff;
		if (ddr_level > 0xf) {
			ddr_level = (val[0] >> 24) & 0xff;
			chip_qulitylevel = val[1] & 0xff;
		}
	...
	if (ddr_level == 0 || ddr_level == 0x0A || ddr_level == 0x05 || ddr_level == 0x08 || ddr_level == 0x10 || ddr_level == 0x09 || ddr_level == 0x0F) {
		pDev->V206DEV105.chip.memfreq = 400;

	} else if (ddr_level == 0x03 || ddr_level == 0x06) {
		pDev->V206DEV105.chip.memfreq = 465;
	} else {
		pDev->V206DEV105.chip.memfreq = 533;
	}

	pDev->V206DEV105.chip.V206DEV038 = 512;
	pDev->V206DEV105.chip.V206DEV039 = 512;
	pDev->V206DEV105.chip.memconf = V206CONFIG015;

	is7201chip = (ddr_level == 0x05)
		|| (ddr_level == 0x08)
		|| (ddr_level == 0x0A);

	V206DEV005("[INFO] ddrlevel: %d.\n", ddr_level);
	V206DEV005("[INFO] is7201: %d.\n", is7201chip);
	if (is7201chip) {
